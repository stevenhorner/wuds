...
def log_nodered(timestamp, bssid, rssi, essid):
    wuds = WUDS_NAME
    print ("Time: %s BSSID: %s RSSI: %s ESSID: %s " % (timestamp, bssid, rssi, essid, wuds))
    # print URL_LOGGING_SITE % (bssid, rssi, essid, wuds)
    node_red_url = URL_LOGGING_SITE % (bssid, rssi, essid.replace(" ", "%20"), wuds)
    res = urllib2.urlopen(node_red_url)
...
def packet_handler(pkt):
    ...
    if ftype == 0 and stype == 4:
        ...
        log_nodered(timestamp=datetime.now(),bssid=bssid, rssi=rssi, essid=essid)
